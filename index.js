...
const allowedOrigins = ["https://startup.cs260party.click"];
...
apiRouter.delete("/user", DB.deleteUser);
...
apiRouter.post("/overrideAspectStatus", async (req, res) => {
  ...
  const { dueDate, unitId, roomName, itemName, aspectName, newStatus } =
    req.body;
  console.log(
    "Overriding aspect status:",
    dueDate,
    unitId,
    roomName,
    itemName,
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Error overriding aspect status",
...
apiRouter.get("/analyze/:dueDate", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: result, message: "Analysis completed successfully." });
  } catch (error) {
